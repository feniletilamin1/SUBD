<?php 
    ...
    $id = $_GET['id'];
    $request = "SELECT object_id FROM object_workers WHERE worker_id = '$id'";
    ...
    $workerFullName = $worker['worker_last_name'] . " " . $worker['worker_first_name'] . " " . $worker['worker_middle_name'];
?>
        ...
        <div class="main-container">
            ...
            <div class="info">
                <div class="info__wrapper">
                    <h1 class="info__title">Отчёт по сотруднику: <?=$workerFullName?></h1>
                    <div class="info__table-wrapper">
                        ...
                        <?php
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $request = "SELECT * FROM objects WHERE id = '$objectId'";
                                $result2 = mysqli_query($link, $request);
                                $object = mysqli_fetch_assoc($result2);
                            ?>
                                <tr class="info__table-row">
                                <td class="info__table-column"><?=$counter?></td>
                                <td class="info__table-column"><?=$object['object_name']?></td>
                                <td class="info__table-column"><?=$object['object_category']?></td>
                                <td class="info__table-column"><?=$object['object_status']?></td>
                                <td class="info__table-column"><?=$object['object_amount']?> руб.</td>
                                <td class="info__table-column"><?=$object['object_phone']?></td>
                                <td class="info__table-column"><?=$object['object_index']?></td>
                                <td class="info__table-column"><?=$object['object_region']?></td>
                                <td class="info__table-column"><?=$object['object_city']?></td>
                                <td class="info__table-column"><?=$object['object_street']?></td>
                                <td class="info__table-column"><?=$object['object_home']?></td>
                                <td class="info__table-column">
                                    <a href="objectsPhotos.php?id=<?=$object['id']?>" class="button button--blue">Фотографии</a>
                                </td>
                                <td class="info__table-column">
                                    <a href="objectWorkers.php?id=<?=$object['id']?>" class="button button--blue">Список сотрудников</a>
                                </td>
                            <?php
                                $counter++;
                            }                                      
